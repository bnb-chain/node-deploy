...
rialtoHash=`cat $src/init.log|grep "Successfully wrote genesis state"|awk -F"hash=" '{print $NF}'|awk '{print $1}'`
...
function start() {
  ...
  --override.fermi ${PassedForkTime} --override.osaka ${PassedForkTime} --override.mendel ${PassedForkTime} --override.pasteur ${PassedForkTime} --override.jenner ${LastHardforkTime} \
